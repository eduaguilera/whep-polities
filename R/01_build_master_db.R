...
polity_codes <- read_csv(
  file.path(whep_dir, "polity_codes.csv"),
  show_col_types = FALSE
)
verified <- read_csv(
  file.path(whep_dir, "polities_database_verified.csv"),
  show_col_types = FALSE
)
cshapes_raw <- read_csv(
  file.path(whep_dir, "cshapes.csv"),
  show_col_types = FALSE
)
faostat <- read_csv(
...
common_names <- read_csv(
  file.path(whep_dir, "common_names.csv"),
  show_col_types = FALSE
)
...
classify_type <- function(name, prefix, code) {
  ...
  if (prefix %in% fao_region_prefixes) {
    return("region")
  }
  ...
  empire_names <- c(
    ...
    "Netherlands (old)",
    "Luxembourg (old)",
    "Netherlands Antilles (original)"
  )
  if (name %in% empire_names) {
    return("aggregate")
  }
  ...
  if (
    str_detect(
      name,
      "^(Eastern|Western|Northern|Southern|Middle|South-|North )"
    )
  ) {
    return("region")
  }
  if (
    str_detect(
      name,
      "^(OECD|Annex|Non-Annex|Least |Low[ -]|High-|Upper-|Net Food|Land Locked|Small Island|Developed|Developing|Sub-Saharan|FAO Major|International Centres|Regional Centres|European Union)"
    )
  ) {
    return("region")
  }
  ...
  if (any(str_detect(name, colonial_keywords))) {
    return("colonial")
  }

  # Historical states (ended before 2025)
  if (
    !is.na(as.integer(str_extract(code, "[0-9]{4}$"))) &&
...
assign_continent <- function(name) {
  ...
  if (any(str_detect(name, fixed(africa_countries)))) {
    return("Africa")
  }
  ...
  if (any(str_detect(name, fixed(asia_countries)))) {
    return("Asia")
  }
  ...
  if (any(str_detect(name, fixed(europe_countries)))) {
    return("Europe")
  }
  ...
  if (any(str_detect(name, fixed(oceania_countries)))) {
    return("Oceania")
  }
  ...
  if (any(str_detect(name, fixed(sa_countries)))) {
  ...
  if (any(str_detect(name, fixed(na_countries)))) {
...
assign_empire <- function(name, start, end) {
  ...
  if (str_detect(name, "Ottoman")) {
    return("Ottoman")
  }
  if (name %in% ottoman_deps) {
    return("Ottoman")
  }

  # Austro-Hungarian
  if (str_detect(name, "Austria-Hungary")) {
    return("Austro-Hungarian")
  }

  # Russian/Soviet
  if (str_detect(name, "USSR|Russia|Soviet")) {
    return("Russian/Soviet")
  }
  soviet_successors <- c(
    "Armenia",
    "Azerbaijan",
  ...
  if (name %in% soviet_successors && start >= 1991) {
    return("Russian/Soviet (successor)")
  }

  # British
  if (str_detect(name, "^British|United Kingdom")) {
    return("British")
  }

  # French
  if (str_detect(name, "^French")) {
    return("French")
  }

  # German states -> German Empire
  if (name %in% german_states) {
    return("German Confederation")
  }

  # Italian states -> Kingdom of Italy
  if (name %in% italian_states) {
    return("Italian pre-unification")
  }

  # Yugoslav
  if (str_detect(name, "Yugoslavia")) {
    return("Yugoslav")
  }
  yugo_successors <- c(
    "Croatia",
  ...
  if (name %in% yugo_successors && start >= 1991) {
    return("Yugoslav (successor)")
  }
...
master <- master %>%
  mutate(
    iso3_code = case_when(
      ...
      str_detect(prefix, "^[A-Z]{3}$") & !prefix %in% fao_region_prefixes ~
        prefix,
      TRUE ~ iso3_code
    )
  )
