...
if (nrow(gaps) > 0) print(head(gaps, 20))

cat("\n=== Timeline Overlaps ===\n")
cat("Found", nrow(overlaps), "overlaps\n\n")
if (nrow(overlaps) > 0) print(head(overlaps, 20))
...
p_sources <- master %>%
  ...
  labs(
    ...
    x = "Number of sources", y = "Count", fill = "Type"
  ) +
  theme_minimal(base_size = 12) +
  theme(legend.position = "top")

ggsave(file.path(plot_dir, "08_source_coverage.png"), p_sources,
...
p_area <- entities %>%
  ...
  labs(
    ...
    subtitle = paste("Polities with area data:",
                     sum(!is.na(entities$area_km2))),
    x = "Area (km²)", y = "Count", fill = "Type"
  ) +
  theme_minimal(base_size = 12) +
  theme(legend.position = "top")

ggsave(file.path(plot_dir, "09_area_distribution.png"), p_area,
...
if (nrow(true_collisions) > 0) print(head(true_collisions %>% select(prefix, n_polities, names), 20))
...
p_verif <- master %>%
  ...
  labs(
    ...
    x = "Status", y = "Count", fill = "Type"
  ) +
  theme_minimal(base_size = 12) +
  theme(legend.position = "top")

ggsave(file.path(plot_dir, "10_verification_status.png"), p_verif,
       width = 10, height = 7, dpi = 150)
...
expected_modern <- c(
  ...
  "Chile", "Colombia", "Comoros", "Congo", "Costa Rica", "Croatia", "Cuba",
  ...
  "Sri Lanka", "Sudan", "Suriname", "Sweden", "Switzerland", "Taiwan",
...
if (length(missing_modern) > 0) cat(paste(missing_modern, collapse = ", "), "\n")
...
p_continent <- ggplot(continent_summary,
...
ggsave(file.path(plot_dir, "11_continent_coverage.png"), p_continent,
       width = 10, height = 7, dpi = 150)
...
key_prefixes <- c("AFG", "CHN", "DEU", "ETH", "FRA", "GBR", "IND",
                  "ROU", "TUR", "USA", "RUS", "BRA", "EGY")
...
p_timeline <- ggplot(timeline_data,
                     aes(xmin = start_year, xmax = end_year,
                         ...
                         fill = polity_type)) +
  geom_rect(aes(ymin = as.numeric(reorder(polity_label, -start_year)) - 0.4,
...
ggsave(file.path(plot_dir, "12_key_prefix_timelines.png"), p_timeline,
