...
## Long-span rows: when territory changed mid-span

A row whose territory demonstrably changed inside its own span, held by a single
polygon vintage, matches early-period data to a territory that did not yet
exist. Issue 22 asked what to do about it and left three options open: split
anyway with an estimate, keep one row and quantify the error, or declare the
early years unusable. The convention, decided 2026-08-14, is **not one of the
three but an order between them**:

1. **Split wherever an observed polygon exists for the sub-period, even if the
   containing row already documents the approximation.** A documented caveat is
   not a substitute for a polygon the repository already ships. `CAP-1800-1910`
   carried a caveat naming the 1895 British Bechuanaland annexation while
   CShapes held the post-1895 step for gwcode 561 all along — the caveat had
   been standing in for a 115,300 km² correction that cost nothing to make.
   Splitting is the answer whenever the source exists, and finding out whether
   it does is part of writing the caveat.
2. **Where no source exists, keep one row and state the error's DIRECTION and
   MAGNITUDE on the page** — not merely that one exists. `polygon_status:
   polygon_vintage_drift` is the machine-readable half and obliges the prose
   half. Territory-basis classification is published for consumers by
   `pipelines/polity-autoimprove/04_territory_basis.py` (`territory_basis`,
   `territory_assumed`), so an area-normalised analysis can filter on
   `assumed_constant` without reading any page.
3. **Never invent a split with an estimated polygon** where the candidate
   sources disagree with the entity being modelled. Checking is cheap and has
   changed the answer: Cliopatria's "British Cape Colony" has 39 pre-1886
   time-steps and looked like the missing source, but its 1877-1884 polygons
   include Pretoria (the annexed Transvaal) and exclude Kimberley (Griqualand
   West, annexed 1880) — adopting them would have inverted the documented error
   rather than removed it. The rejection is recorded on the page, so the next
   agent does not re-propose the same source.

Option (c) — marking early years unusable — was **not** adopted: the repository
never withholds a row it can describe, and a quantified one-directional error is
more useful than a hole.
