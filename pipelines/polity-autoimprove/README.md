...
| `state/magnitude_outliers.csv` | assertions whose reported output is implausible for the territory they are matched to, from the deterministic magnitude screen (`05_magnitude_screen.py`) | #29 |
| `state/magnitude_impossible_area.csv` | the subset of that screen where the implied area is impossible rather than merely large — a positive production against a zero or near-zero area (`05_magnitude_screen.py`) | #29, #420 |
| `state/land_containment.csv` | the FAO-1952 land-use inequality: what must fit INSIDE arable land, cross-block (`13_land_containment.py`) | #29 |
| `state/trade_availability_summary.csv` | the third quantity a trade mirror lacks — production — and what it decides about an entrepôt flow (`13_trade_entrepot_direction.py`) | #112 |
| `state/audit_sample_banked.csv` | a sampled slice of routed labels with their method, ISO/name agreement and risk flags, for auditing match confidence (`review_stats.py --audit-sample N`). **Gitignored and regenerable** — a blank human-audit template, not a measurement; a scored one is committed under a new name per `state/.gitignore`. This row named `state/audit_sample.csv` until 2026-08-25, which is the OLD name: nothing writes it, and the copy some working trees still carry is a leftover | #7 |
| `state/era_shift_verdicts.csv` | per-row verdicts for the IIA tobacco/hops era, including `impossible_yield_zero_area` — a zero area beside positive production is a VERDICT, not a skip (`29_era_shift_verdicts.py`) | #416, #459 |
| `state/collapse_groups.csv` | groups the consumer reduces with `mean(value)` because its key carries no source and no label, partitioned into true duplicate keys and item-code collisions (`30_consumer_key_collapse.py`) | #451, #457 |
| `state/cross_label_duplication.csv` | two labels of ONE source byte-identical across a contiguous BLOCK of years and plainly different outside it — one series copied into the other for a stretch (`31_cross_label_duplication.py`) | #468, #470 |
| `state/grid_ambiguous_zeros.csv` | published zeros the source's reporting grid cannot distinguish from a small positive value, where the series lives at the grid's resolution floor (`32_grid_ambiguous_zeros.py`) | #446 |
| *(no table)* `35_retest_data_errors.py` | re-runs the QUANTITATIVE claims in `data_errors.csv` against the current raw extract and panel — row counts, label counts, absences — and fails if one no longer reproduces. The mirror of `11_retest_conventions.py` for the other curated registry, which had no re-test at all while six gates consumed it. Covers only the 4 entries resting on a reproducible figure; the other 25 are single attributable cells with no measurement behind them and are out of scope by design | #431 |
| `state/cell_attribution.csv` | `38_cell_attribution.py` — per-cell provenance for the series `item_provenance` can only call `unattributable`. 1,329 cells, 107 series, 52 multi-label. Re-finds #372's Ghana split and the `us puerto rico` origin of the US coffee series adjudicated on #424. Carries `series_time_structure`: 22 series `partition` time (a succession), 30 `interleaved` (a concurrent whole/part, or a persistent column header whose dates contradict the entity's), so a reader knows which rows a date-keyed repair could even apply to. **Keys on the NORMALISED item**: matching the panel's `cacao, beans` against item_provenance's `cacao beans` silently excluded 48 of 134 series and 934 values, leaving no trace because nothing present was wrong | #372, #443 |
| `state/source_value_precision.csv` | `37_value_precision.py` — the 1000-grid, 100-grid and sub-unit shares of every (source, unit) with 500+ non-zero values, plus an era split for `iia` alone. Turns issue 446's prose conventions into a joinable table, derived from the panel rather than transcribed, so the two cannot drift. `mitchell` `ha`/`heads` 100.0% on a 1000-grid; `iia` `ha` 96.3% from 1934 against 9.8% before; `fao1952` fine throughout. Zero values are excluded — 0 is a multiple of every grid | #446 |
| `state/label_hierarchy_identity.csv` | `36_label_hierarchy_identity.py` — for every label that is a space-or-colon-separated prefix of others in the same source, tests `sum(parts) / whole` in the cells carrying both. Turns "is this a whole and its parts, or the same territory twice?" from an argument into an arithmetic identity: Germany's three FAO subdivisions sum to the printed Reich in **28 of 28** cells, French North Africa's three in 4 of 4, Indonesia's three panels in 6 of 6, while `windward islands` reaches only 62% and `iia` `ethiopia pdr` EQUALS `ethiopia`. Keyed on the exact SET of parts present, since Western+Eastern and Western+Berlin are both "2 of 3" and give opposite answers. A 3-cell floor with one narrow exemption for identities that are already proof — `korea` (108.9 = 98.9 + 10.0, ONE cell) and `british borneo` (770 = 35+295+440, 953 = 47+335+571) | #411, #449, #450 |
| `state/component_underselection.csv` | nutrient series that consistently publish the SMALLEST of several raw materials the source prints side by side — invisible to #379's oscillation gate (`33_component_underselection.py`) | #490 |
