...
def main() -> int:
    ...
    pow10_all = sum(is_clean_power_of_ten(r) for r in (extreme["hi"] / extreme["lo"]))
    rows = [
        ...
        ("investigable_ratio_is_pow10", int(out["ratio_is_pow10"].sum())),
    ...
    print(f"  ratio within {POW10_WINDOW:.0%} of a power of ten: "
          f"{int(out['ratio_is_pow10'].sum()):,} of {len(out):,} "
