...
edges <- read_csv(
  file.path(analysis_dir, "knowledge_graph_edges.csv"),
  show_col_types = FALSE
)
nodes <- read_csv(
  file.path(analysis_dir, "knowledge_graph_nodes.csv"),
  show_col_types = FALSE
)
polities <- read_csv(
...
p1 <- ggplot(
  rel_counts,
  aes(x = reorder(relation, n), y = n, fill = relation)
) +
  ...
  coord_flip() +
  labs(
    title = "Knowledge Graph: Edge Count by Relation Type",
    x = NULL,
    y = "Number of edges"
  ) +
...
ggsave(
  file.path(plot_dir, "kg_01_relation_type_distribution.png"),
  p1,
  width = 12,
  height = 6,
  dpi = 150
...
p2a <- ggplot(polity_nodes, aes(x = total_degree)) +
  ...
  geom_vline(
    xintercept = median(polity_nodes$total_degree),
    color = "red",
    linetype = "dashed"
  ) +
  annotate(
    "text",
    x = median(polity_nodes$total_degree) + 2,
    y = Inf,
    vjust = 2,
...
p2b <- ggplot(
  top20,
  aes(
    x = reorder(str_trunc(polity_name, 30), total_degree),
    y = total_degree,
    fill = polity_type
  )
) +
...
ggsave(
  file.path(plot_dir, "kg_02_degree_distribution.png"),
  p2,
  width = 14,
  height = 6,
  dpi = 150
...
p3 <- ggplot(
  chain_data,
  aes(x = start_year, y = fct_rev(iso_label), color = polity_type)
) +
  geom_segment(
    aes(xend = end_year, yend = fct_rev(iso_label)),
    linewidth = 4,
    alpha = 0.7
  ) +
  geom_point(size = 2) +
  ...
  labs(
    title = "Temporal Successor Chains for Selected Countries",
    x = "Year",
    y = NULL,
    color = "Type"
  ) +
...
ggsave(
  file.path(plot_dir, "kg_03_successor_chains.png"),
  p3,
  width = 16,
  height = 8,
  dpi = 150
...
if (nrow(col_edges) > 0) {
  col_nodes <- bind_rows(
    nodes %>%
      filter(polity_code %in% col_edges$source) %>%
      mutate(role = "metropole"),
    nodes %>%
      filter(polity_code %in% col_edges$target) %>%
      mutate(role = "colony")
  ) %>%
  ...
  metro_counts <- col_edges %>%
    ...
    inner_join(
      nodes %>% select(polity_code, polity_name),
      by = c("source" = "polity_code")
    )

  col_plot_data <- col_edges %>%
    inner_join(
      nodes %>% select(polity_code, polity_name, continent),
      by = c("target" = "polity_code")
    ) %>%
    inner_join(
      nodes %>% select(polity_code, polity_name),
      by = c("source" = "polity_code"),
      suffix = c("_colony", "_metro")
    )

  p4 <- ggplot(
    col_plot_data,
    aes(
      x = reorder(str_trunc(polity_name_metro, 20), -table(source)[source]),
      fill = continent
    )
  ) +
    geom_bar() +
    ...
    labs(
      title = "Colonial Empire Networks",
      x = "Metropole",
      y = "Number of colonies/dependencies",
  ...
  p4 <- ggplot() +
    annotate("text", x = 0.5, y = 0.5, label = "No colonial edges") +
    theme_void()
}

ggsave(
  file.path(plot_dir, "kg_04_colonial_empires.png"),
  p4,
  width = 14,
  height = 8,
  dpi = 150
...
sub_data <- edges %>%
  ...
  inner_join(
    polities %>% select(polity_code, polity_name, iso3_code),
    by = c("source" = "polity_code")
  ) %>%
  inner_join(countries, by = c("target" = "parent_code"))

p5 <- ggplot(
  sub_data %>% count(label, name = "n_regions"),
  ...
  labs(
    title = "Subnational Hierarchy: Admin-1 Units per Country",
    x = NULL,
    y = "Number of admin-1 units"
  ) +
  theme_minimal(base_size = 12) +
  theme(plot.title = element_text(face = "bold"))

ggsave(
  file.path(plot_dir, "kg_05_subnational_hierarchy.png"),
  p5,
  width = 10,
  height = 6,
  dpi = 150
...
p6a <- ggplot(
  polity_nodes %>% count(polity_type),
...
p6b <- ggplot(
  polity_nodes,
  aes(x = in_degree, y = out_degree, color = polity_type)
) +
...
p6c <- ggplot(
  temporal_edges %>% filter(!is.na(period)),
...
ggsave(
  file.path(plot_dir, "kg_06_graph_metrics_summary.png"),
  p6,
  width = 14,
  height = 12,
  dpi = 150
