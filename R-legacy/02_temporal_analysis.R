...
events <- full_join(formations, dissolutions, by = "year") %>%
  ...
  pivot_longer(
    cols = c(formations, dissolutions),
    names_to = "event_type",
    values_to = "count"
  ) %>%
...
p1 <- ggplot(events, aes(x = year, y = count, fill = event_type)) +
  ...
  scale_fill_manual(
    values = c("formations" = "#2196F3", "dissolutions" = "#F44336"),
...
ggsave(
  file.path(plot_dir, "01_formation_dissolution_events.png"),
  p1,
  width = 14,
  height = 7,
  dpi = 150
...
active_counts <- tibble(year = years) %>%
  ...
  mutate(
    n_total = sum(
      entities$start_year <= year & entities$end_year >= year,
      na.rm = TRUE
    ),
    n_sovereign = sum(
...
p2 <- ggplot(active_long, aes(x = year, y = count, fill = type)) +
  ...
  scale_x_continuous(breaks = seq(1800, 2025, 25)) +
  geom_vline(
    xintercept = c(1871, 1918, 1945, 1960, 1991),
    linetype = "dashed",
    alpha = 0.5
  ) +
  annotate(
    "text",
    x = c(1871, 1918, 1945, 1960, 1991),
...
ggsave(
  file.path(plot_dir, "02_active_polities_timeline.png"),
  p2,
  width = 14,
  height = 7,
  dpi = 150
...
p3 <- entities %>%
  ...
  labs(
    ...
    subtitle = paste(
      "Median:",
      median(entities$duration, na.rm = TRUE),
      "years"
    ),
    x = "Duration (years)",
    y = "Count",
...
ggsave(
  file.path(plot_dir, "03_polity_lifespan_distribution.png"),
  p3,
  width = 12,
  height = 7,
  dpi = 150
...
era_counts <- eras %>%
  ...
  mutate(
    n_active = sum(
      entities$start_year <= era_end &
        entities$end_year >= era_start,
      na.rm = TRUE
    ),
    n_formed = sum(
      entities$start_year >= era_start &
        entities$start_year <= era_end,
      na.rm = TRUE
    ),
    n_dissolved = sum(
...
ggsave(
  file.path(plot_dir, "04_era_breakdown.png"),
  p4,
  width = 12,
  height = 7,
  dpi = 150
...
ggsave(
  file.path(plot_dir, "05_empire_dissolutions.png"),
  p5,
  width = 12,
  height = 7,
  dpi = 150
...
p6 <- ggplot(top_long, aes(x = duration, y = polity_name)) +
  ...
  geom_text(
    aes(label = paste0(start_year, "-", end_year)),
    hjust = -0.1,
    size = 2.5
  ) +
  scale_fill_brewer(palette = "Set2") +
...
ggsave(
  file.path(plot_dir, "06_longest_sovereign_polities.png"),
  p6,
  width = 12,
  height = 10,
  dpi = 150
...
short_lived <- entities %>%
  ...
  arrange(duration) %>%
  mutate(
    polity_label = paste0(polity_name, " (", start_year, "-", end_year, ")")
  )
...
p7 <- short_lived %>%
  ...
  labs(
    ...
    subtitle = paste(
      "Total:",
      nrow(short_lived),
      "polities with duration <= 25 years"
    ),
    x = "Duration (years)",
    y = NULL,
    fill = "Type"
...
ggsave(
  file.path(plot_dir, "07_short_lived_polities.png"),
  p7,
  width = 12,
  height = 12,
  dpi = 150
