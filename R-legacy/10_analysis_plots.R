...
p_temporal <- ggplot(active_counts, aes(x = year, y = count)) +
  ...
  geom_vline(
    xintercept = c(1886, 1914, 1945, 1991),
    linetype = "dashed",
    alpha = 0.4
  ) +
  annotate(
    "text",
    x = c(1886, 1914, 1945, 1991),
    y = max(active_counts$count),
    label = c("CShapes\nstart", "WWI", "WWII", "USSR\nfall"),
    vjust = -0.2,
    size = 3
  ) +
  labs(
    title = "Active Polities Over Time (excl. regions & subnational)",
    x = "Year",
    y = "Active polities"
  ) +
...
ggsave(
  file.path(plot_dir, "temporal_01_active_polities.png"),
  p_temporal,
  width = 14,
  height = 6,
  dpi = 150
...
events <- bind_rows(
  non_sub %>%
    transmute(year = start_year, event = "formation", type = polity_type),
  non_sub %>%
...
p_events <- ggplot(events, aes(x = year, fill = event)) +
  ...
  scale_fill_manual(
    values = c(formation = "#4CAF50", dissolution = "#F44336")
  ) +
  labs(
    title = "Formation and Dissolution Events",
    x = "Year",
    y = "Count",
    fill = NULL
  ) +
...
ggsave(
  file.path(plot_dir, "temporal_02_events.png"),
  p_events,
  width = 14,
  height = 6,
  dpi = 150
...
p_lifespan <- ggplot(non_sub, aes(x = duration_years, fill = polity_type)) +
  ...
  labs(
    title = "Polity Lifespan Distribution",
    x = "Duration (years)",
    y = "Count",
    fill = "Type"
  ) +
...
ggsave(
  file.path(plot_dir, "temporal_03_lifespan.png"),
  p_lifespan,
  width = 12,
  height = 6,
  dpi = 150
...
ggsave(
  file.path(plot_dir, "temporal_04_types_over_time.png"),
  p_type_time,
  width = 14,
  height = 6,
  dpi = 150
...
p_cont_type <- ggplot(
  non_sub,
  aes(x = fct_infreq(continent), fill = polity_type)
) +
  geom_bar() +
  scale_fill_manual(values = TYPE_COLORS) +
  labs(
    title = "Polity Distribution by Continent and Type",
    x = NULL,
    y = "Count",
    fill = "Type"
  ) +
...
ggsave(
  file.path(plot_dir, "dist_01_continent_type.png"),
  p_cont_type,
  width = 12,
  height = 6,
  dpi = 150
...
ggsave(
  file.path(plot_dir, "dist_02_verification.png"),
  p_verif,
  width = 10,
  height = 6,
  dpi = 150
...
if (has_polys) {
  ...
  poly_meta <- polys %>%
    ...
    st_drop_geometry() %>%
    left_join(
      df %>% select(polity_code, polity_type, continent, polygon_source),
      by = "polity_code"
    )

  # Area distribution by source
  p_area <- ggplot(
    poly_meta %>% filter(!is.na(polygon_source)),
    aes(x = area_deg2, fill = str_trunc(polygon_source, 20))
  ) +
    geom_histogram(bins = 50, alpha = 0.7) +
    scale_x_log10() +
    labs(
      title = "Polygon Area Distribution by Source",
      x = "Area (approx deg^2, log scale)",
      y = "Count",
      fill = "Source"
    ) +
  ...
  ggsave(
    file.path(plot_dir, "polyqual_01_area_dist.png"),
    p_area,
    width = 12,
    height = 6,
    dpi = 150
  ...
  p_frag <- ggplot(poly_meta, aes(x = n_parts)) +
    ...
    scale_x_log10() +
    labs(
      title = "Polygon Fragmentation (number of parts)",
      x = "Parts (log scale)",
      y = "Count"
    ) +
    theme_minimal(base_size = 12) +
  ...
  ggsave(
    file.path(plot_dir, "polyqual_02_fragmentation.png"),
    p_frag,
    width = 10,
    height = 6,
    dpi = 150
...
p_iso <- ggplot(
  iso_shared %>% slice_max(n_entries, n = 30),
  ...
  labs(
    title = sprintf(
      "Top 30 Most-Shared ISO3 Codes (%d codes shared total)",
      nrow(iso_shared)
    ),
    x = "ISO3 code",
    y = "Number of entries"
...
ggsave(
  file.path(plot_dir, "iso_01_shared_codes.png"),
  p_iso,
  width = 10,
  height = 8,
  dpi = 150
...
p_prefix <- ggplot(
  prefix_groups,
  aes(x = reorder(prefix, n_isos), y = n_isos)
) +
  geom_col(fill = "#EF5350", alpha = 0.8) +
  coord_flip() +
  labs(
    title = sprintf("Prefix Collision Groups (%d groups)", nrow(prefix_groups)),
    x = "Code prefix",
    y = "Number of distinct ISO3 codes"
  ) +
  theme_minimal(base_size = 11) +
  theme(plot.title = element_text(face = "bold"))

ggsave(
  file.path(plot_dir, "iso_02_prefix_collisions.png"),
  p_prefix,
  width = 10,
  height = 8,
  dpi = 150
...
p_sources <- ggplot(source_coverage, aes(x = reorder(sources, n), y = n)) +
  ...
  coord_flip() +
  labs(
    title = "Polity Coverage by Data Source",
    x = NULL,
    y = "Number of entries"
  ) +
  theme_minimal(base_size = 12) +
...
ggsave(
  file.path(plot_dir, "coverage_01_by_source.png"),
  p_sources,
  width = 10,
  height = 6,
  dpi = 150
...
p_poly_src <- ggplot(poly_sources, aes(x = reorder(polygon_source, n), y = n)) +
  ...
  coord_flip() +
  labs(
    title = "Polygon Source Distribution",
    x = NULL,
    y = "Number of entries"
  ) +
  theme_minimal(base_size = 12) +
...
ggsave(
  file.path(plot_dir, "coverage_02_polygon_sources.png"),
  p_poly_src,
  width = 12,
  height = 6,
  dpi = 150
